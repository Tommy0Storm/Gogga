...
BRIDGE_BUILD = "2025-12-21T-dockerstats-swarm-statsError-v2"
...
async def _safe_send(ws: WebSocketServerProtocol, payload: Dict[str, Any]) -> None:
    if ws.closed:
        return
    try:
...
async def _send_terminal(ws: WebSocketServerProtocol, line: str) -> None:
    await _safe_send(ws, {"type": "terminal", "line": line, "ts": int(time.time() * 1000)})


async def _send_error(ws: WebSocketServerProtocol, message: str) -> None:
...
class Session:
    ...
    async def ensure_ssh(self) -> Optional[asyncssh.SSHClientConnection]:
        ...
        if not self.target_host or not self.username or not self.password:
            await _send_error(self.ws, "Missing target/auth (need hello.target.host + auth.username/password)")
            return None
    ...
    try:
        async for raw in ws:
            ...
            if mtype == "auth":
                ...
                print(f"[ws] auth user={session.username} target={session.target_host} peer={peer}")
                await _send_terminal(ws, "Auth received")
