...
> = {
  backend: {
    start:
      "cd /home/ubuntu/Dev-Projects/Gogga/gogga-backend && nohup uvicorn app.main:app --host 0.0.0.0 --port 8000 > /tmp/gogga-backend.log 2>&1 &",
    stop: 'pkill -f "uvicorn app.main:app" || true',
    restart:
      'pkill -f "uvicorn app.main:app" || true; sleep 2; cd /home/ubuntu/Dev-Projects/Gogga/gogga-backend && nohup uvicorn app.main:app --host 0.0.0.0 --port 8000 > /tmp/gogga-backend.log 2>&1 &',
  },
  cepo: {
    start:
      'ssh gogga-worker "cd ~/DEV-Drive/docker/compose && docker compose up -d cepo" || echo "Note: CePO runs on worker server"',
    stop: 'ssh gogga-worker "cd ~/DEV-Drive/docker/compose && docker compose stop cepo" || echo "Note: CePO runs on worker server"',
    restart:
      'ssh gogga-worker "cd ~/DEV-Drive/docker/compose && docker compose restart cepo" || echo "Note: CePO runs on worker server"',
  },
  frontend: {
    start:
      "cd /home/ubuntu/Dev-Projects/Gogga/gogga-frontend && nohup pnpm dev:http > /tmp/gogga-frontend.log 2>&1 &",
    stop: 'pkill -f "next dev -H 0.0.0.0" 2>/dev/null || true; fuser -k 3000/tcp 2>/dev/null || true',
    restart:
      'pkill -f "next dev -H 0.0.0.0" 2>/dev/null || true; fuser -k 3000/tcp 2>/dev/null || true; sleep 2; cd /home/ubuntu/Dev-Projects/Gogga/gogga-frontend && nohup pnpm dev:http > /tmp/gogga-frontend.log 2>&1 &',
  },
  admin: {
    start:
      "cd /home/ubuntu/Dev-Projects/Gogga/gogga-admin && nohup npm run dev > /tmp/gogga-admin.log 2>&1 &",
    stop: 'pkill -f "next dev -H 0.0.0.0 -p 3100" || true; fuser -k 3100/tcp 2>/dev/null || true',
    restart:
      'pkill -f "next dev -H 0.0.0.0 -p 3100" || true; fuser -k 3100/tcp 2>/dev/null || true; sleep 2; cd /home/ubuntu/Dev-Projects/Gogga/gogga-admin && nohup npm run dev > /tmp/gogga-admin.log 2>&1 &',
  },
};

// Check if user has service admin privileges
async function checkServiceAdminAuth(
  request: NextRequest
  ...
  try {
    ...
    if (!user.isServiceAdmin && !user.isAdmin) {
      return {
        authorized: false,
        error: "Insufficient permissions - requires service admin access",
      };
    }
...
export async function POST(
  ...
  const command =
    SERVICE_COMMANDS[service][action as "start" | "stop" | "restart"];
  ...
  try {
    ...
    // Log to console
    console.log(
      `[ADMIN] Service action by ${auth.email}: ${action} ${service}`
    );
